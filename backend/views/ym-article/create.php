...
<?php $this->beginPage() ?>
	...
	<script type="text/javascript">
		...
		layui.use(['form', 'layer', 'upload', 'vip_tab', 'colorpicker'], function () {
			...
			var titleStyle = {};
            // $("input[name=is_show][value=1]").attr("checked", rejectSet == 1 ? true : false);
    	    // $("input[name=is_show][value=0]").attr("checked", rejectSet == 0 ? true : false);
            
    	    // 动态生成的对象无法更新渲染需要重新设置属性
		    // $("input[name=reject][value="+rejectSet+"]").prop("checked", "checked");
            // $("input[name=is_show][value='1']").prop("checked",true);
            // $("input[name=is_show][value='1']").prop("checked",true);
            form.render();
...
//				color: '#c71585',
			...
			form.on('submit(form-submit)', function (data) {
				...
				var url = '<?= Yii::$app->urlManager->createUrl("ym-article/create")?>';
				$.post(
					url,
					formData,
					function (result) {
